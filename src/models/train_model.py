...
from src.utils.tf_gpu_debug import (
    enable_gpu_debug_logging,
    profile_training_step,
    print_gpu_operation_support_info,
    analyze_model_device_placement
)
from src.utils.tf_training_monitor import BatchTimingCallback
...
def parse_args():
    ...
    parser.add_argument('--no-mlflow', action='store_true',
                        help='Completely disable MLflow tracking (overrides other MLflow options)')
...
# Enable GPU debugging logs with appropriate verbosity
gpus = enable_gpu_debug_logging(verbose=args.verbose)
logger.info(f"GPU debugging enabled: Found {len(gpus) if gpus else 0} GPUs")
    ...
    if gpus:
        try:
            ...
            if len(gpus) > 1 and not args.single_gpu:
                logger.info(f"Enabling multi-GPU training across {len(gpus)} GPUs")
        ...
        logger.warning("No GPUs detected - using CPU for training (this will be slower)")
...
def load_and_preprocess_data(data_path, test_size=0.2, val_size=0.25, random_state=42, sampling_technique='none', sampling_ratio=0.5, k_neighbors=5):
    ...
    if data_path.endswith('.parquet'):
        ...
        if using_multi_gpu:
            ...
            logger.info(f"Using {num_threads} parallel threads for data loading")
        ...
        if using_multi_gpu:
            ...
            logger.info(f"Using chunked reading with size {chunksize} for CSV data")
            chunks = []
    ...
    if df[target_col].dtype == 'bool':
        ...
        logger.info(f"Converted boolean target to integer: {target_col}")
    ...
    logger.info(f"Using {len(numeric_cols)} numeric features: {numeric_cols}")

    # Check for and handle NaN values
    nan_counts = df[numeric_cols].isna().sum()
    if nan_counts.sum() > 0:
        logger.warning(f"Found {nan_counts.sum()} NaN values across {sum(nan_counts > 0)} columns")
        logger.info(nan_counts[nan_counts > 0])

        # Fill NaN values with column means
        logger.info("Filling NaN values with column means")
        df[numeric_cols] = df[numeric_cols].fillna(df[numeric_cols].mean())

    # Check for transaction_frequency feature, add if missing
    if 'transaction_frequency' not in df.columns:
        logger.warning("'transaction_frequency' feature not found in dataset. Adding with default values.")
    ...
    logger.info(f"Unique classes in dataset: {unique_classes}")
    ...
    if len(unique_classes) > 1:
        ...
        if sampling_technique != 'none':
            ...
            if sampling_func is not None:
                ...
                logger.warning(f"Unknown sampling technique '{sampling_technique}'. Using original data.")
        else:
            logger.info("\nNo advanced sampling technique applied.")

        # Print class distribution in training data
        unique, counts = np.unique(y_train, return_counts=True)
        logger.info(f"Class distribution in training data: {dict(zip(unique, counts))}")
        logger.info(f"Fraud ratio in training data: {np.sum(y_train == 1) / len(y_train):.4f}")
    else:
        # Use regular split if only one class (no stratification needed)
        logger.warning("Only one class present. Using regular train/test split without stratification.")
        X_train_val, X_test, y_train_val, y_test = train_test_split(
...
def save_model_artifacts(model, model_type, scaler, feature_names, params, results, model_dir=None):
    ...
    if os.path.exists(standard_model_path) or os.path.islink(standard_model_path):
        try:
            ...
            logger.warning(f"Could not remove existing model file: {e}")
    ...
    try:
        ...
        logger.info(f"Created symbolic link: {standard_model_path} -> {os.path.basename(model_path)}")
    except Exception as e:
        ...
        try:
            ...
            logger.info(f"Copied model to standardized path: {standard_model_path}")
        except Exception as e2:
            logger.error(f"Could not copy model file: {e2}")
    ...
    logger.info(f"Model artifacts saved with both timestamped and standardized names in {model_dir}")
...
def evaluate_classification_model(model, X_test, y_test):
    ...
    logger.info(f"Unique classes in test data: {unique_classes}")
    ...
    if len(unique_classes) > 1:
        ...
        logger.warning("Only one class present in test data. AUC cannot be calculated.")
...
def get_loss_function_for_model(loss_function_name, focal_gamma=2.0, focal_alpha=0.25, class_weights=None):
    ...
    try:
        ...
        logger.info(f"Using custom loss function: {loss_function_name} with parameters: {loss_params}")
        return loss
    except Exception as e:
        logger.warning(f"Error configuring custom loss function: {str(e)}")
        logger.info("Falling back to binary_crossentropy")
        return 'binary_crossentropy'
...
def train_classification_model(X_train, y_train, X_val, y_val, input_dim, hidden_layers=[64, 32], learning_rate=0.001, batch_size=64, epochs=50, class_weight=None, model_path=None, loss_function='binary_crossentropy', focal_gamma=2.0, focal_alpha=0.25, l2_regularization=0.0):
    ...
    if 'strategy' in globals() and strategy is not None:
        logger.info(f"Creating classification model with multi-GPU strategy")
    ...
    if strategy is not None and hasattr(strategy, 'scope'):
        ...
        # Analyze which operations will run on GPU vs CPU
        analyze_model_device_placement(model)

    # Callbacks
    class EpochEndCallback(Callback):
        def on_epoch_end(self, epoch, logs=None):
            logs = logs or {}
            # Only print a summary at the end of each epoch
            metrics_str = ' - '.join(f'{k}: {v:.4f}' for k, v in logs.items())
            logger.info(f"Epoch {epoch+1} complete: {metrics_str}")
    
    callbacks = [
        EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True),
        BatchTimingCallback(),  # Add GPU monitoring callback
        EpochEndCallback(),     # Custom minimal per-epoch reporting
    ]
    ...
    if 'strategy' in globals() and strategy is not None and hasattr(strategy, 'num_replicas_in_sync'):
        ...
        logger.info(f"Using effective batch size of {effective_batch_size} for {strategy.num_replicas_in_sync} GPUs")
    elif tf.config.list_physical_devices('GPU'):
        # Apple Silicon GPU detection and optimization
        gpus = tf.config.list_physical_devices('GPU')
        is_apple_silicon = any('Metal' in gpu.name for gpu in gpus)

        if is_apple_silicon:
            # Apple M-series GPUs benefit from larger batch sizes
            multiplier = 4  # Default multiplier for Apple Silicon

            # Try to get multiplier from args if available
            if 'args' in globals():
                multiplier = getattr(args, 'batch_size_multiplier', 4)

            effective_batch_size = batch_size * multiplier
            logger.info(f"🍏 Apple Silicon GPU detected: Using optimized batch size of {effective_batch_size} (base: {batch_size} × multiplier: {multiplier})")
        else:
    ...
    # Profile a single training batch to diagnose GPU utilization
    if isinstance(train_dataset, tf.data.Dataset):
        logger.info("\nProfiling a single training batch...")
        for x_batch, y_batch in train_dataset.take(1):
            profile_training_step(model, x_batch, y_batch)
            break
    else:
        # If not using tf.data.Dataset
        batch_size_to_profile = min(effective_batch_size if 'effective_batch_size' in locals() else batch_size, 32)
        logger.info(f"\nProfiling a single training batch (size={batch_size_to_profile})...")
        profile_training_step(model, X_train[:batch_size_to_profile], y_train[:batch_size_to_profile])

    if 'strategy' in globals() and strategy is not None and hasattr(strategy, 'num_replicas_in_sync'):
        # Train model with optimized datasets for multi-GPU
        history = model.fit(
            train_dataset,
        ...
        with tf.device('/GPU:0'):
            # Use effective_batch_size even for single GPU training when defined
            history = model.fit(
                X_train, y_train,  # Training data
                validation_data=(X_val, y_val),
                epochs=epochs,
                batch_size=effective_batch_size if 'effective_batch_size' in locals() else batch_size,
                callbacks=callbacks,
                class_weight=class_weight,
                verbose=0
            )
...
def run_classification_experiment(X_train, X_val, X_test, y_train, y_val, y_test,
    ...
    # Only set up MLflow if it's enabled and an experiment name is provided
    if use_mlflow and experiment_name:
        mlflow.set_experiment(experiment_name)
        run_context = mlflow.start_run(run_name=params['run_name'])
    else:
        # Create a dummy context manager when MLflow is not used
    ...
    with run_context:
        ...
        if 'class_weight' not in params or params['class_weight'] is None:
            ...
            if 'class_weight_ratio' in params and params['class_weight_ratio'] is not None:
                ...
                logger.info(f"Using specified class weight ratio: {ratio}")
        ...
        # Log loss function parameters if MLflow is enabled
        if use_mlflow:
        ...
        # Log evaluation metrics if MLflow is enabled
        if use_mlflow:
...
def train_autoencoder_model(X_train, X_val, input_dim, batch_size=256, epochs=20,
    ...
    # Use multi-GPU strategy if available
    if 'strategy' in globals() and strategy is not None:
        logger.info(f"Creating autoencoder model with multi-GPU strategy")
        with strategy.scope():
            # Build encoder
            input_layer = Input(shape=(input_dim,))
    ...
    # Profile a single training batch to diagnose GPU utilization
    if isinstance(train_dataset, tf.data.Dataset):
        logger.info("\nProfiling a single autoencoder training batch...")
        for x_batch in train_dataset.take(1):
            profile_training_step(model, x_batch, x_batch)  # Input = output for autoencoders
            break
    else:
        # If not using tf.data.Dataset
        batch_size_to_profile = min(effective_batch_size if 'effective_batch_size' in locals() else batch_size, 32)
        logger.info(f"\nProfiling a single autoencoder training batch (size={batch_size_to_profile})...")
        profile_training_step(model, X_train[:batch_size_to_profile], X_train[:batch_size_to_profile])
...
def main():
    ...
    # Safely check if no_mlflow attribute exists (for backward compatibility)
    if hasattr(args, 'no_mlflow') and args.no_mlflow:
        use_mlflow = False
        logger.info("MLflow tracking explicitly disabled via --no-mlflow flag")
    ...
    if model_type in ['classification', 'both']:
        ...
        classification_params = {
            'experiment_name': experiment_name if use_mlflow else None,
            'run_name': run_name,
