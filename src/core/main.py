...
def load_config(config_path='config.yaml'):
    ...
    try:
        ...
        config_dir_path = os.path.join('config', config_path)
        if os.path.exists(config_dir_path):
            with open(config_dir_path, 'r') as file:
                config = yaml.safe_load(file)
            logger.info(f"Configuration loaded from {config_dir_path}")
            return config
        ...
        if os.path.exists(script_config_path):
            with open(script_config_path, 'r') as file:
                config = yaml.safe_load(file)
            logger.info(f"Configuration loaded from {script_config_path}")
            return config
            
        raise FileNotFoundError(f"Configuration file not found at {config_path}, {config_dir_path}, or {script_config_path}")
...
def run_pipeline(config_path='config.yaml', steps=None, model_type='both', disable_gpu=False, single_gpu=False):
    ...
    """
    command = f"python src/core/pipeline.py --config {config_path}"
...
def run_model_training(config, model_type='both', disable_gpu=False, single_gpu=False, batch_size_multiplier=None, memory_growth=False):
    ...
    command = f"python src/models/training/train_model.py --data-path {processed_data_path} --model-dir {model_dir} --model-type {model_type}"
...
def run_model_evaluation(config, model_type='both', disable_gpu=False, single_gpu=False, memory_growth=False):
    ...
    if model_type in ['classification', 'both']:
        ...
        if not classification_models:
            ...
            command = (
                f"python src/models/evaluation/evaluate_model.py "
...
def deploy_model(config, disable_gpu=False, single_gpu=False, memory_growth=False):
    ...
    command = f"python src/models/deployment/save_model_artifacts.py --model-dir {model_dir} --output-dir {deployment_dir}{gpu_flags}"
    run_command(command)
    
    # Then, start the API server
    command = f"python src/models/deployment/deploy_model.py --model-dir {deployment_dir} --host 0.0.0.0 --port 8080{gpu_flags}"
    return run_command(command)
...
def run_streaming(config, disable_gpu=False, single_gpu=False, memory_growth=False):
    ...
    command = f"python src/spark_jobs/streaming/streaming_fraud_detection.py --model-dir {model_dir}"
...
def monitor_model(config):
    ...
    command = (
        f"python src/models/monitoring/model_monitoring.py "
